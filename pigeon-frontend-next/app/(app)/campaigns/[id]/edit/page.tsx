...
import { useCampaign, useUpdateCampaign, useCampaignStats, useCampaignStatsByTemplate } from "@/hooks/useCampaigns";
...
import { normalizeScheduleWeekdaysFromApi, normalizeTimezoneForSelect } from "@/lib/campaign-editor/edit-helpers";
import type { CampaignEmailStep, CampaignFormData } from "@/lib/campaign-editor/types";
...
export default function EditCampaignPage() {
  ...
  const { data: campaignStats } = useCampaignStats(campaignId || "");
  const { data: statsByTemplate, isLoading: statsByTemplateLoading } = useCampaignStatsByTemplate(campaignId || "");
  ...
  const [replyToType, setReplyToType] = useState<"default" | "none" | "gmail" | "imap" | "custom">("default");
  ...
  const hasRun = (campaignStats?.sent ?? 0) > 0;

  useEffect(() => {
    if (!campaign || initialized) return;
    const seq = campaign.email_sequence && campaign.email_sequence.length > 0
      ? campaign.email_sequence
      : (campaign.template_ids || []).map((tid, i) => ({ template_id: tid, delay_days: i === 0 ? 0 : i * 2 }));
    const byDelay = new Map<number, string[]>();
    for (const step of seq) {
      const d = step.delay_days ?? 0;
      if (!byDelay.has(d)) byDelay.set(d, []);
      byDelay.get(d)!.push(step.template_id);
    }
    const delays = Array.from(byDelay.keys()).sort((a, b) => a - b);
    const emails: CampaignEmailStep[] = delays.map((d) => ({
      templateIds: byDelay.get(d) ?? [""],
      delay: d,
    }));
    if (emails.length === 0) {
      setCampaignData((prev) => ({
        ...prev,
        name: campaign.name,
        contactList: campaign.contact_list_ids?.[0] ?? "",
        timezone: normalizeTimezoneForSelect(campaign.timezone),
        dailyLimit: campaign.daily_limit ?? 50,
        startTime: campaign.start_time ?? "09:00",
        endTime: campaign.end_time ?? "17:00",
        scheduleWeekdays: normalizeScheduleWeekdaysFromApi(campaign.schedule_weekdays),
        senderType: campaign.sender_type as "gmail" | "smtp",
        senderIds: campaign.sender_ids ?? [],
        senderRotation: (campaign.sender_rotation as "round_robin" | "random") ?? "round_robin",
        enableRotation: campaign.rotation_enabled ?? (campaign.sender_ids?.length ?? 0) > 1,
        useAiGeneration: campaign.use_ai_generation ?? false,
        aiGenerationPrompt: campaign.ai_generation_prompt ?? "",
        aiGenerationProvider: campaign.ai_generation_provider ?? "",
        useExternalEnrichment: campaign.use_external_enrichment ?? false,
        externalEnrichmentPrompt: campaign.external_enrichment_prompt ?? "",
        externalEnrichmentProvider: campaign.external_enrichment_provider ?? "",
        campaignRealEngagementNetwork: campaign.campaign_real_engagement_network ?? false,
        campaignPersonalNetworkPool:
          (campaign.campaign_real_engagement_network ?? false) &&
          (campaign.campaign_personal_network_pool ?? false),
        campaignRealEngagementPercent: campaign.campaign_real_engagement_percent ?? 20,
        openTracking: campaign.open_tracking ?? true,
        emails: [{ templateIds: [""], delay: 0 }],
      }));
      setReplyToType(campaign.reply_to_type ?? "default");
      setReplyToId(campaign.reply_to_id ?? null);
    ...
    setCampaignData({
      name: campaign.name,
      inbox: "",
      senderType: (campaign.sender_type as "gmail" | "smtp") || "gmail",
      senderIds: campaign.sender_ids ?? [],
      senderRotation: (campaign.sender_rotation as "round_robin" | "random") ?? "round_robin",
      timezone: normalizeTimezoneForSelect(campaign.timezone),
      contactList: campaign.contact_list_ids?.[0] ?? "",
    ...
    setReplyToEmail(campaign.reply_to_email ?? "");
    setInitialized(true);
  }, [campaign, templates, initialized]);

  // When inboxes load and campaign uses SMTP, set root domain selection from senders (all unique roots) or default
  useEffect(() => {
    if (!initialized || !inboxes.length || campaignData.senderType !== "smtp") return;
    if (smtpSelectedRootDomains.length > 0) return;
    if (!domains.length) return;

    if (campaignData.senderIds.length > 0) {
      const roots = new Set<string>();
      for (const id of campaignData.senderIds) {
        const inbox = inboxes.find((i) => i.id === id);
        if (inbox?.domain_id) {
          const d = domainById.get(inbox.domain_id);
          if (d?.domain) roots.add(getRootDomain(d.domain));
        }
      }
      if (roots.size > 0) {
        setSmtpSelectedRootDomains([...roots].sort());
        return;
      }
    }
    if (!allowSmtpDomainAutofillRef.current) return;
    const first = smtpRootDomainOptions.find((o) => o.hasReadyInboxes)?.rootDomain;
    if (first) setSmtpSelectedRootDomains([first]);
  }, [
    initialized,
    inboxes,
    domains.length,
    domainById,
    campaignData.senderType,
    campaignData.senderIds,
    smtpSelectedRootDomains.length,
  ...
  useEffect(() => {
    ...
    api.analytics
      .getBestSendTime(userId, campaignData.timezone, 30, campaignId || undefined)
      .then((res) => {
        if (cancelled) return;
  ...
  }, [userId, campaignData.timezone, campaignId]);
  ...
  const handleSendConnectionTest = async () => {
    ...
    try {
      ...
      toast.error((error as { message?: string })?.message || "Failed to send test email");
    } finally {
      setIsTestingConnection(false);
    }
  };

  const buildEditPayload = (): Campaign | null => {
    ...
    return {
      ...
      start_time: campaignData.startTime ?? campaign.start_time ?? "09:00",
      end_time: campaignData.endTime ?? campaign.end_time ?? "17:00",
      schedule_weekdays: campaignData.scheduleWeekdays,
      timezone: campaignData.timezone ?? campaign.timezone ?? "America/New_York",
      sender_type: campaignData.senderType,
      sender_ids: campaignData.senderIds,
      sender_rotation: campaignData.senderRotation ?? "round_robin",
  ...
  const handleSaveDraft = async () => {
    if (!campaignId || !campaign) {
      toast.error("Campaign not loaded. Please wait or refresh the page.");
      return;
    }
    const draft = validateCampaignForm(campaignData, "draft", {
  ...
  const handleSaveCampaign = async () => {
    ...
    try {
      ...
      toast.error((error as { message?: string })?.message || "Failed to update campaign");
    }
  };
  ...
  const renderStep = () => {
    switch (currentStep) {
              ...
              <CardHeader className="pb-2">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <Badge variant="secondary" className="text-[10px] h-5 shrink-0">
                    {hasRun ? "Fixed" : "Editable until first send"}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent className="space-y-6 pt-0">
                <div className="space-y-2">
                  <Label htmlFor="edit-audience-contact-list">Contact list</Label>
                  ...
                  >
                    <SelectTrigger
                      id="edit-audience-contact-list"
                      className={hasRun ? "bg-muted/50 opacity-100 cursor-not-allowed border-dashed" : undefined}
                    >
                  ...
                  <p className="text-[11px] text-muted-foreground italic">
                    {hasRun
                      ? "Note: Contact list cannot be changed after the campaign has started sending."
                      : "You can still change the contact list because this campaign has not sent any emails yet."}
                  </p>
                </div>
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        ...
                        <strong>{spreadDays} day{spreadDays !== 1 ? "s" : ""}</strong> for best deliverability.
                      </p>
                  ...
                  >
                    ...
                    <div className="rounded-lg border bg-background/80 p-3 sm:p-4">
                      <div className="flex gap-3">
                        <Checkbox
                          id="edit-campaign-personal-network-pool"
                        ...
                        <div className="min-w-0 space-y-2">
                          <Label
                            htmlFor="edit-campaign-personal-network-pool"
                            className={cn(
                              "text-sm font-medium",
              ...
              <>
                  ...
                  return (
                    <Card key={index}>
                      <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-base">{index === 0 ? "Initial Email" : `Follow-up ${index}`}</CardTitle>
                        {index > 0 && (
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setCampaignData({ ...campaignData, emails: campaignData.emails.filter((_, i) => i !== index).map((e) => ({ ...e, templateIds: e.templateIds ?? [""] })) })}>
                      ...
                      <CardContent className="space-y-4">
                        {templateIds.map((tid, vi) => (
                          <div key={vi} className="space-y-2">
                            <Label>{index === 0 && vi === 0 ? "Template" : `Variant ${String.fromCharCode(65 + vi)}`}</Label>
                            <Select
                              value={tid || ""}
                              onValueChange={(value) => {
                                const newEmails = campaignData.emails.map((em, i) => ({ ...em, templateIds: em.templateIds ?? [""] }));
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setCampaignData({
                      ...campaignData,
                      emails: [...campaignData.emails.map((e) => ({ ...e, templateIds: e.templateIds ?? [""] })), { templateIds: [""], delay: (last?.delay ?? 0) + 2 }],
                    });
                  }}
                >
      ...
      case 5: {
        const sendingInboxes =
          campaignData.senderType === "gmail"
            ? inboxes.filter((i) => i.sender_type === "gmail" && campaignData.senderIds.includes(i.id))
            : inboxes.filter((i) => campaignData.senderIds.includes(i.id));
        return (
          <CampaignReviewContent
            ...
            sendingDisplay={{
              type: "inboxes",
              rows: sendingInboxes.map((i) => ({ id: i.id, email: i.email })),
            }}
            editContext={
              campaign
                ? {
                    ...
                    statsByTemplate,
                    campaignStats,
                    firstStepTemplateCount: campaignData.emails[0]?.templateIds?.length ?? 0,
                  }
                : undefined
            }
          />
    ...
    return (
      <div className="max-w-3xl mx-auto bg-background min-h-[calc(100vh-4rem)]">
        <div className="flex items-center gap-4 mb-8">
          <Button variant="ghost" size="icon" onClick={() => router.push("/campaigns")}><ArrowLeft className="w-5 h-5" /></Button>
          <div>
  ...
  return (
    <div className="max-w-3xl w-full mx-auto bg-background min-h-[calc(100vh-4rem)]">
      <div className="flex items-center gap-4 mb-8">
        <Button variant="ghost" size="icon" onClick={() => router.push("/campaigns")}><ArrowLeft className="w-5 h-5" /></Button>
        <div>
          ...
          {steps.map((step, index) => (
            <React.Fragment key={step.id}>
              <div
                ...
                data-tour={
                  step.title === "Basics"
                    ? "campaigns-edit-tab-info"
                    : step.title === "Audience"
                      ? "campaigns-edit-tab-audience"
                      : step.title === "Email sequence"
                        ? "campaigns-edit-tab-sequence"
                        : step.title === "Delivery"
                          ? "campaigns-edit-tab-delivery"
                          : step.title === "Review"
                            ? "campaigns-edit-tab-review"
                            : undefined
                }
              >
                <div className={`flex items-center justify-center w-10 h-10 rounded-full transition-colors ${currentStep >= step.id ? "gradient-primary text-white" : "bg-secondary text-muted-foreground"}`}>
                  {currentStep > step.id ? <Check className="w-5 h-5" /> : <step.icon className="w-5 h-5" />}
                </div>
                <span className={`text-xs mt-2 whitespace-nowrap ${currentStep >= step.id ? "text-foreground font-medium" : "text-muted-foreground"}`}>{step.title}</span>
              </div>
              {index < steps.length - 1 && <div className={`h-[2px] flex-1 mx-4 rounded-full ${currentStep > step.id ? "bg-primary" : "bg-secondary"}`} />}
            </React.Fragment>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-6 pb-12">
        ...
        {currentStep === 5 ? (
          <Button className="gradient-primary" onClick={handleSaveCampaign} disabled={updateCampaign.isPending}>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={handleSaveDraft} disabled={updateCampaign.isPending}>
              {updateCampaign.isPending ? (
                <>
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                  Saving...
